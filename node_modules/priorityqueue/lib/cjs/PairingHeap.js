...
function traverse(node) {
  if (!node) return [];
  return [node.value].concat(_toConsumableArray(traverse(node.firstChild)), _toConsumableArray(traverse(node.nextSibling)));
}
/** mutate first argument */


function mergeNode(a, b, comp) {
  if (!a || !b) return a || b;
...
function mergeChildren(firstChild, comp) {
  ...
  while (cursor) {
    ...
    first.nextSibling = null; // isolate second node & move cursor
    ...
    first = mergeNode(first, second, comp); // collect merged siblings in reversed order
  ...
  while (lastSibling) {
    ...
    lastSibling = revCursor.nextSibling; // merge last pair
...
function (_PriorityQueue) {
  _inherits(PairingHeap, _PriorityQueue);
  ...
  function PairingHeap() {
    ...
    _this = _possibleConstructorReturn(this, (_getPrototypeOf2 = _getPrototypeOf(PairingHeap)).call.apply(_getPrototypeOf2, [this].concat(args)));
  ...
  _createClass(PairingHeap, [{
    ...
    value: function from(array) {
      ...
      var instance = new PairingHeap(option);
